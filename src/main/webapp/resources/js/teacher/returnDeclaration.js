layui.use(['form', 'layer', 'laydate', 'table', 'laytpl'], function () {
    ...
    var tableIns = table.render({
        elem: '#returnDeclarationList',
        id: "returnDeclarationListTable",
        url: '/teacher/order',
        cellMinWidth: 95,
        height: "full-125",
        page: true,
        ...
        cols: [[
            ...
            {
                field: 'gradingStandardContent', title: '评分标准',  align: "center", sort: true
            },
            {field: 'gradingBasis', title: '评分依据', align: "center", sort: true},
            {
                field: 'auditId', title: '审核机构',  align: "center", templet: function (d) {
            ...
            {
                field: 'certifiedTime', title: '退回时间', align: 'center',sort: true, templet: function (d) {
    ...
    function updateReturnDeclaration(data) {
        var index = layui.layer.open({
            ...
            area: ['40%', '40%'],
            content: "returnDeclarationEdit.html",
    ...
    function deleteReturnDeclaration(data) {
    ...
    form.on("submit(searchForm)", function (data) {
        table.reload('returnDeclarationListTable', {
            page: {
                curr: 1 //重新从第 1 页开始
            },
            where: data.field
    ...
    $("#delAllBtn").click(function () {
        var checkStatus = table.checkStatus('returnDeclarationListTable'),
            data = checkStatus.data;

        if (data.length > 0) {
            deleteReturnDeclaration(data);
        } else {
    ...
    //后端排序
    table.on('sort(returnDeclarationList)', function (obj) { //returnDeclarationList是table原始容器的属性 lay-filter="对应的值"
        table.reload('returnDeclarationListTable', {
            page: {
                curr: 1 //重新从第 1 页开始
            },
    ...
    //列表操作
    table.on('tool(returnDeclarationList)', function (obj) {
        var layEvent = obj.event,
            data = obj.data;

        if (layEvent === 'edit') { //编辑
            updateReturnDeclaration(data);
